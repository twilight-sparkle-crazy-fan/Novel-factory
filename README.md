...
打开 `EXPERIMENTAL_MATERIAL_SYSTEM=true` 后，会启用 `/api/experimental/material-system` 下的实验 API，并在小说资料库里显示“导出分析包 / 导入分析包 / 重建实验资料 / 提示词预算 / 预算设置 / 确认队列”。当前支持导出 `.llm4pkg`、导入前校验、纯新文档导入、合并到当前 TXT、替换当前实验资料层，并可按资料层选择只导入语义观察、时间线、人物/关系、确认队列或预算配置。系统能把现有章节摘要、人物卡和结构化事实投影为实验时间线、人物实体、人物关系、确认队列和提示词预算报告。开启实验开关后，章节分析还会额外抽取统一事件并写入实验事件账本，确认队列可创建缺失人物实体并把事件或关系写回实验资料层，实验资料视图可编辑时间线、人物、关系，支持人物别名和人物合并；正式生成会使用实验提示词预算器产出的资料段，预算设置会按分段裁剪资料注入。导入前校验会显示包内资料层记录数、目标 TXT hash 对照、本地相同原文匹配和记录级差异预览；合并导入会保留已人工编辑/确认的人物、时间线节点、时间线事件和关系边字段，并把包内字段差异写入确认队列；从队列逐字段套用包内值仍未实现。

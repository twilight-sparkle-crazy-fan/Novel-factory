...
打开 `EXPERIMENTAL_MATERIAL_SYSTEM=true` 后，会启用 `/api/experimental/material-system` 下的实验 API，并在小说资料库里显示“导出分析包 / 导入分析包 / 重建实验资料 / 提示词预算 / 预算设置 / 确认队列”。当前支持导出 `.llm4pkg`、导入前校验、纯新文档导入、合并到当前 TXT、替换当前实验资料层，并可按资料层选择只导入语义观察、时间线、人物/关系、确认队列、辅助账本或预算配置；合并导入还可填写章节范围，只导入该范围内有明确来源的资料记录。系统能把现有章节摘要、人物卡和结构化事实投影为实验时间线、人物实体、人物关系、确认队列、辅助账本和提示词预算报告。开启实验开关后，章节分析还会额外抽取统一事件并写入实验事件账本，确认队列可创建缺失人物实体并把事件或关系写回实验资料层，位置、物件和悬念类辅助观察可从队列写入辅助账本和时间线，能力观察可写入人物经历；实验资料视图可新建、编辑、删除时间线节点、时间线事件、人物实体、人物事实、人物阶段档案、人物经历事件、关系边、关系事件和地点/物件/悬念辅助账本，支持人物别名、人物合并和人物拆分，拆分时可选择把关系边及其关系事件迁移到新人物；人物事实、近期经历和辅助账本会进入提示词预算段。正式生成会使用实验提示词预算器产出的资料段，预算设置会按分段裁剪资料注入。导入前校验会显示包内资料层记录数、目标 TXT hash 对照、本地相同原文匹配和记录级差异预览；合并导入会保留已人工编辑/确认的人物、时间线节点、时间线事件、关系边和辅助账本字段，把包内字段差异写入确认队列，并可从队列应用包内冲突字段。

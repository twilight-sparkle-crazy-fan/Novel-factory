...
它不是普通聊天壳子，而是围绕“本地写小说”做的工具：续写、重生成抽卡、TXT 前文整理、人物卡、下一章大纲和最新稿导出。
...
打开 `EXPERIMENTAL_MATERIAL_SYSTEM=true` 后，会启用 `/api/experimental/material-system` 下的实验 API，并在小说资料库里显示“导出分析包 / 导入分析包 / 迁移分析包 / 重建实验资料 / 提示词预算 / 当前快照 / 预算设置”。
...
7. 如需测试实验资料系统，先设置 `EXPERIMENTAL_MATERIAL_SYSTEM=true` 并重启，再测试 `.llm4pkg` 导出、校验、纯新导入、合并导入和章节范围替换导入。

...
- 实验资料系统：支持 `.llm4pkg` 分析包、分层时间线、人物阶段档案、关系网络、确认队列和提示词预算器，默认关闭，可用环境变量开启。
...
打开 `EXPERIMENTAL_MATERIAL_SYSTEM=true` 后，会启用 `/api/experimental/material-system` 下的实验 API，并在小说资料库里显示“导出分析包 / 导入分析包 / 迁移分析包 / 重建实验资料 / 提示词预算 / 当前快照 / 预算设置 / 确认队列”。

当前已支持：

- `.llm4pkg` 分析包导出、导出前报告、导入前校验和旧 schema 迁移。
- 纯新文档导入、合并到当前 TXT、替换当前实验资料层。
- 按资料层选择导入语义观察、时间线、人物/关系、确认队列、辅助账本或预算配置。
- 合并和替换导入时填写章节范围，只处理该范围内有明确来源的资料记录。
- manifest、documents、章节、chunk、provenance、资料记录、内部引用和包内文件 hash 校验。
- 记录级差异预览，以及本地人工编辑字段保护；冲突字段会进入确认队列，可勾选字段后应用包内值。
- 把现有章节摘要、人物卡和结构化事实投影为实验时间线、人物实体、人物关系、确认队列、辅助账本和提示词预算报告。
...
- 确认队列按状态/类型筛选、全选当前待确认、清空选择、批量确认普通项、批量忽略。
...
3. 在设置里调整“自动续写目标 token”、词汇风格和词表白名单，生成一段较长正文。
4. 导入一份 TXT，检查拆章、分片总结、人物卡、结构化事实和提示词预览。
5. 生成并选用下一章大纲，再导出 Markdown / JSON 备份。
6. 把满意正文加入资料库，分别测试“只保存待总结”和“加入后立即总结”。
7. 如需测试实验资料系统，先设置 `EXPERIMENTAL_MATERIAL_SYSTEM=true` 并重启，再测试 `.llm4pkg` 导出、校验、纯新导入、合并导入、章节范围替换导入和确认队列。

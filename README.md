...
打开 `EXPERIMENTAL_MATERIAL_SYSTEM=true` 后，会启用 `/api/experimental/material-system` 下的实验 API，并在小说资料库里显示“导出分析包 / 导入分析包 / 重建实验资料 / 提示词预算 / 预算设置 / 确认队列”。当前支持导出 `.llm4pkg`、导入前校验、纯新文档导入、合并到当前 TXT、替换当前实验资料层，并能把现有章节摘要、人物卡和结构化事实投影为实验时间线、人物实体、人物关系、确认队列和提示词预算报告。开启实验开关后，章节分析还会额外抽取统一事件并写入实验事件账本，确认队列可创建缺失人物实体并把事件或关系写回实验资料层，正式生成会使用实验提示词预算器产出的资料段；预算设置会按分段裁剪资料注入。更细的人物合并、别名确认和局部导入 UI 仍在开发中。
